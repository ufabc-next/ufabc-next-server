...
Model.pre('save', async function () {
  ...
  if (this.isNew) {
    let enrollment = await this.constructor
      .findOne({
    ...
    if (enrollment)
      throw new errors.BadRequest(
        `Você só pode comentar uma vez neste vínculo: ${this.enrollment}`
      )
  }
})
...
Model.static(
  ...
  async function (
    ...
    if (!userId)
      throw new errors.BadRequest(`Usuário não encontrado: ${userId}`)
    ...
    await Promise.all(
      response.map(async (r) => {
        r.myReactions = {
          like: !!(await Reactions.count({
            comment: String(r._id),
            user: String(userId),
            kind: 'like',
          })),
          recommendation: !!(await Reactions.count({
            comment: String(r._id),
            user: String(userId),
            kind: 'recommendation',
          })),
          star: !!(await Reactions.count({
            comment: String(r._id),
            user: String(userId),
            kind: 'star',
          })),
